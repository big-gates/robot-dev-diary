...
double bearing_to(double rx, double ry, double tx, double ty) {
    return std::atan2(ty - ry, tx - rx);
}

double relative_bearing(double rx, double ry, double rtheta, double tx, double ty) {
...
bool in_fov(double rx, double ry, double rtheta, double tx, double ty, double fov_rad) {
  return std::abs(relative_bearing(rx, ry, rtheta, tx, ty)) <= fov_rad / 2;
}